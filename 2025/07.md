...
AI
Fluencyのシリーズ記事（[AI Fluency: Deep Dive 1: What is Generative AI?](https://www.anthropic.com/ai-fluency/deep-dive-1-what-is-generative-ai)）から、ジェネレーティブAIの基本概念を学んだ。

### ジェネレーティブAIとは何か？

この記事は、ジェネレーティブAI（生成AI）の基本概念について説明している。

### 主要なポイント

ジェネレーティブAIは、既存のデータを分析するだけでなく、新しいコンテンツ（テキスト、画像、コード）を生成するAIである。

現代の大規模言語モデル（LLM）が実現可能になったのは、以下の3つの重要な発展によるもの：

- アルゴリズムとアーキテクチャの革新（特にトランスフォーマーアーキテクチャ）
- 大量のデジタル学習データ
- 計算能力の劇的な向上
...
最も効果的なアプリケーションは、人間とAIの強みを組み合わせたものである。

人間が批判的思考、判断力、創造性、倫理的監督を提供することが重要である。
