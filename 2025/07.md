...
1. **Docker Compose**: 実際のDBエンジンをローカルで動かす最も実用的な方法
2. **インメモリDB**: SQLite、Redis Memory Serverを使った軽量テスト環境
3. **クラウド互換サービス**: LocalStack（AWS）、Firebase Emulator（GCP）
4. **データダンプ**: pg_dump、mongodumpを使った本番データの移行

開発フェーズに応じた使い分けが重要で、開発環境では実際のDBエンジン、単体テストではインメモリDB、統合テストではDocker環境を使用するのが効果的。
...
async function dumpDatabase() {
  const command = `pg_dump -h ${host} -U ${user} -d ${database} --data-only --inserts > dump.sql`;
  await execAsync(command);
}

// テスト環境セットアップ
beforeAll(async () => {
  await docker.upAll({ cwd: __dirname });
  await seedDatabase();
});
```
