...
## 2025-07-09: Claude CodeにMCPサーバをセットアップしてみた

Claude Codeでの`mcp add`コマンドを使用して、MCPサーバをセットアップする方法を学習。

### MCPサーバの追加

Claude Codeには、Model Context Protocol
(MCP) サーバを追加する機能が組み込まれており、`mcp add`コマンドで簡単にサーバを追加できる。
...
Context7は文書検索とライブラリドキュメント参照機能を提供するMCPサーバで、以下の機能を持つ：

- **ライブラリID解決**: パッケージ名からContext7互換のライブラリIDを取得
- **ドキュメント検索**: 指定したライブラリの最新ドキュメントを取得
- **トークン制限**: 最大10,000トークンまでのドキュメントを取得可能
- **トピック指定**: 特定のトピック（例：hooks、routing）に焦点を当てたドキュメント取得
...
設定後、Claude Codeでは以下のような機能が利用可能になる：

- `mcp__context7__resolve-library-id`: ライブラリ名からIDを解決
- `mcp__context7__get-library-docs`: ライブラリドキュメントを取得

### 設定ファイルの構造

`.mcp.json`ファイルが自動生成され、以下のような設定が保存される：
