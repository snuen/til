...
## 2025-06-09: コミットメッセージを自動生成したい

JetBrains IDE は使っていないので、VS Code で使える方法を調べたメモ。

### VS Code + GitHub Copilot

- **使い方**: 変更をステージ → Source Control ビューのコミットメッセージ欄右上にある ✨（sparkles）ボタンをクリック
- **メモ**: ステージした差分だけを OpenAI モデルに送り、Conventional Commits 風に要約して提案してくれる
  <https://github.blog/ai-and-ml/github-copilot/github-for-beginners-essential-features-of-github-copilot/>

**メリット**

- 拡張機能を入れるだけですぐ使える（Copilot Chat 拡張 v0.22 以降に同梱）
- 差分のみを送信するため情報リークを最小化
- ボタン 1 クリックで完結、学習コストほぼゼロ

**デメリット**

- GitHub Copilot のサブスクリプション料金がかかる（個人 Pro/Business プランなど）
- オフラインでは利用できない

### VS Code + GitLens (GitKraken AI)

- **使い方**: 変更をステージ → Source Control パネル上部の **Generate Commit Message (AI)** ボタン、または
  `GitLens: Generate Commit Message with AI` コマンドを実行
- **メモ**: 先頭行にサマリー、2 行目以降に詳細という 2 段構成で生成され、編集して確定できる
  [oai_citation:1‡help.gitkraken.com](https://help.gitkraken.com/gitlens/gl-gk-ai/)

**メリット**

- VS Code 拡張 **GitLens** をインストールするだけで導入可能
- コミット差分を見ながらワンクリック生成 → そのまま修正して確定できる

**デメリット**

- AI 機能を使うには **GitKraken アカウント**でサインインし、GitLens Pro 以上（Teams /
  Enterprise）のサブスクリプションが必要
- インターネット接続前提のためオフラインでは使えない
