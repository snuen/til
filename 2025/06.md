...
## 2025-06-25: Obsidian Web Clipperを導入してみた

Pocketが廃止されるとのことで、代替ツールとしてObsidian Web Clipperを導入してみた。

Obsidian Web Clipperは、WebページをObsidianに保存し、後で読み返すことができるツール。

読みたいページが積読になってしまっている現代人の一人にとって、何からどう読んで、そこから何を得るかは一つの課題だと思う。

クリップしたページをObsidianで管理することで、以下のようなメリットがある。

- **一元管理**: WebページをObsidianのノートとして保存できるため、情報が散逸しない。
- **タグ付け**: Obsidianのタグ機能を使って、クリップしたページにタグを付けて整理できる。
- **リンク機能**: Obsidianのリンク機能を使って、関連するノート同士をつなげて情報を整理できる。
- **全文検索**: Obsidianの全文検索機能を使って、クリップしたページの内容を簡単に検索できる。
- **Markdown形式**: ObsidianはMarkdown形式でノートを保存するため、クリップしたページもMarkdown形式で保存される。
- **オフラインアクセス**:
  Obsidianはローカルに保存されるため、インターネット接続がなくてもクリップしたページを閲覧できる。
- **カスタマイズ性**: Obsidianのプラグインを使って、クリップしたページの表示や管理方法をカスタマイズできる。
