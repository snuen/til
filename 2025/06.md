...
AWSのサービスは、世界中に分散したデータセンター（リージョン）で提供されている。

各リージョンは独立しており、地理的に離れた場所に配置されているため、災害対策やレイテンシの低減に役立つ。

#### リージョンの選び方

- **地理的要件**: ユーザーの所在地に近いリージョンを選ぶことで、レイテンシを低減できる。
- **法的要件**: データの保管場所に関する法律や規制を考慮する必要がある。
- **サービスの可用性**: 一部のAWSサービスは特定のリージョンでのみ利用可能なため、必要なサービスが提供されているリージョンを選ぶ。
- **コスト**: リージョンごとに料金が異なるため、コスト面も考慮する。

### ゾーン

各リージョンは複数のアベイラビリティゾーン（AZ）に分かれている。

AZは独立した電源・ネットワークを持つデータセンターの集合体で、同一リージョン内で高い可用性を提供する。

#### ゾーンの利用方法

- **高可用性**: アプリケーションを複数のAZに分散配置することで、1つのAZが障害を起こしてもサービスを継続できる。
- **負荷分散**: トラフィックを複数のAZに分散することで、負荷を均等に分散し、パフォーマンスを向上させる。
- **データの冗長化**: データベースやストレージサービスをマルチAZ構成で利用することで、データの可用性を高める。

#### マルチAZ構成

マルチAZ構成では、異なるAZにリソースを配置することで、障害発生時の影響を最小限に抑える。

例えば、RDSやElastiCacheなどのデータベースサービスは、マルチAZ構成で高可用性を実現できる。

### VPC（Virtual Private Cloud）

VPCは、AWS上に仮想的なネットワークを構築するサービス。

VPCを利用することで、独自のIPアドレス範囲やサブネット、ルートテーブル、セキュリティグループなどを設定できる。

#### サブネット

サブネットは、VPC内でIPアドレスの範囲を分割したもの。

サブネットは、パブリックサブネットとプライベートサブネットに分けられる。

- **パブリックサブネット**: インターネットから直接アクセス可能なサブネット。通常、Webサーバーやロードバランサーなどを配置する。
  - インターネットゲートウェイを介してインターネットと接続される。
- **プライベートサブネット**: インターネットから直接アクセスできないサブネット。データベースやアプリケーションサーバーなど、外部からのアクセスが不要なリソースを配置する。
  - NATゲートウェイを介してインターネットと接続される。

AZごとにサブネットを作成し、リソースを配置することで、可用性やセキュリティを向上させることができる。

#### ルートテーブル

ルートテーブルは、VPC内のトラフィックのルーティングを制御する。

ルートテーブルには、サブネットごとにルートを設定できる。

- **インターネットゲートウェイ**: パブリックサブネットからインターネットへのアクセスを可能にする。
- **NATゲートウェイ**: プライベートサブネットからインターネットへのアクセスを可能にする。プライベートサブネットのリソースが外部APIなどにアクセスする際に利用される。

#### セキュリティグループ

セキュリティグループは、VPC内のリソースへのアクセスを制御するファイアウォール。

セキュリティグループは、インスタンスごとに設定でき、以下のようなルールを定義できる。

- **インバウンドルール**: インスタンスへのアクセスを許可するルール。特定のIPアドレスやポート番号からのアクセスを制御できる。
- **アウトバウンドルール**: インスタンスから外部へのアクセスを許可するルール。特定のIPアドレスやポート番号へのアクセスを制御できる。

### VPCの設定

VPCを設定する際の基本的な流れは以下の通り：

1. **VPCの作成**: AWSマネジメントコンソールからVPCを作成
2. **サブネットの作成**: VPC内にパブリックサブネットとプライベートサブネットを作成
