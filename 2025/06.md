...
Cloud）は、AWSが提供する「必要なときに必要なだけ使える」仮想サーバーサービス。物理サーバーを自分で用意することなく、Web画面から簡単にサーバー（インスタンス）を作成・削除できる。主な特徴は以下の通り。
...
5. **ストレージの設定** デフォルトのEBS（仮想ディスク）サイズを確認・変更できます。必要なら追加も可能です。
