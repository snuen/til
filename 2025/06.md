...
## 2025-06-30: 新しくscaffoldしたプロジェクトでClaude Codeを使ってみる

Next.jsの新しいプロジェクトをscaffoldして、Claude Codeを使ってみた。

あと、他にもいくつかtipsを学んだので、まとめておく。

### Initialize your project

プロジェクト全体の構造をClaude Codeに理解させるために、以下のコマンドを実行。

`claude`でClaude CodeのREPLを立ち上げて、
...
コマンドで、`CLAUDE.md`ファイルを、プロジェットガイド用に生成する。

参照）<https://docs.anthropic.com/en/docs/claude-code/setup#initialize-your-project>

### Handling large inputs

プロジェクトが大きくなってきた場合、Claude Codeはコンテキストの制限に達することがある。

なので、下記のルールを守ると良い。

- 大量のテキストの直接のペーストを避ける
- コンテンツを分割して、適切な大きさのファイルに分ける

参照）<https://docs.anthropic.com/en/docs/claude-code/setup#handling-large-inputs>

### Pro tips集

以下、Claude CodeをCCと略す。
...
2. Pass command lien args to CC
...
9. Copy paste images into CC

Take screenshots and than control + v (not command + v) to paste them into CC

10. Give CC mockups

Design a mock and then drag it into CC to ask for implementation

11. Use screenshots as feedback

12. Automate feedback with Playwright
...
20. Use `CLAUDE.md` for project context

21. `/init` to creates `CLAUDE.md`

22. `#` adds to CLAUDE.md

23. `~/.claude/CLAUDE.md` - global

24. Use `CLAUDE.md` file in subdirectories

25. Refactor `CLAUDE.md` often

26. Use Anthropic's prompt improver

27. `/` commands shows CC commands

28. Use args with `/` commands

29. Use `tab` to autocomplete filenames
...
37. or use the GitHub MCP server

38. Ask CC to files PRs
...
42. Consider using `/clear` vs `/compact`

43. Use scratch pads to plan work

44. Use GitHub issues to plan work

45. Small context -> Lower costs

46. Use open telemetry support

47. Use CC max plan

参照）<https://youtu.be/TiNpzxoBPz0?si=Cxq7YulAFPYM5ouU>
