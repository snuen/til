...
## 2025-06-30: 新しくscaffoldしたプロジェクトでClaude Codeを使ってみる（詳細＆実践例）

Next.jsの新規プロジェクトをscaffoldし、Claude Codeを本格的に活用するワークフローを試した。

また、別トピックではあるが、セットアップから実践的な活用例、便利なTipsまで、具体的なコマンドや入出力例を交えてまとめる。

### 1. プロジェクト初期化とClaude Codeセットアップ

#### Next.jsプロジェクトのscaffold

```bash
npx create-next-app@latest my-claude-demo
cd my-claude-demo
```

#### Claude Codeの起動とプロジェクト要約

```bash
claude
```

Claude CodeのREPLで、プロジェクト全体の概要を取得：
...
```txt
This project is a Next.js app scaffolded with create-next-app.
...
参照: [公式セットアップ手順](https://docs.anthropic.com/en/docs/claude-code/setup#initialize-your-project)

### 2. 大規模プロジェクト時の入力制限対策

Claude Codeはコンテキスト制限があるため、巨大なファイルや大量のテキストは分割して扱うのがコツ。

#### ファイル分割例

```bash
split -l 5000 bigfile.js bigfile_part_
```

分割したファイルごとにCCへ入力し、要約や変換を依頼できる。

### 3. Claude Code活用Tips＆コマンド集（実例付き）

#### 1. Bash CLIとして使う

```bash
claude "checkout a new branch named 'feature/new-feature'"
...
```bash
claude "run the tests with --verbose"
# => npm test -- --verbose
```
...
# => There are 27 files in this project.
```

#### 4. 他のCLIと組み合わせる

```bash
claude "list all files in this project" | grep ".js"
# => app/page.tsx
# => next.config.js
```
...
```bash
cat data.csv | claude "parse this CSV and summarize the data"
# => Summary: 100 rows, 5 columns. Top value in column A: 'foo'
```

#### 6. 画像・モックアップ・スクリーンショットの活用

- 画像をドラッグ＆ドロップ、またはCtrl+Vで貼り付けて解析・実装依頼が可能。

#### 7. MCP連携（サーバー/クライアント）

- CCをMCPサーバー/クライアントとして使い、DBやAPIと連携できる。
- 例:

  ```bash
  > Can you see my database?
  # => Yes, I see a PostgreSQL database with 3 tables: users, posts, comments.
  ```

#### 8. CLAUDE.mdの活用

- `/init`で生成、`#`で追記、サブディレクトリやグローバルにも配置可能。
- プロジェクトの文脈・履歴をCCに伝えるのに便利。
...
- `tab`でファイル名補完
- `Esc`で入力キャンセル
- `undo`や`commit`もAIに依頼可能

### 4. Claude Codeでの自動化ワークフロー例

#### 例: 新規Reactコンポーネントの生成からテスト・コミットまで

1. **コンポーネント生成依頼**

   ```bash
   claude "generate a new component called UserCard with props: name, avatarUrl"
   ```
...
2. **テストコード生成依頼**
   ...
   ```bash
   claude "write a unit test for UserCard"
   ```
   ...
   **出力例:**
   ...
   test('renders user name and avatar', () => {
     render(<UserCard name="Alice" avatarUrl="/alice.png" />);
...
3. **テスト実行依頼**
   ...
   **出力例:**
   ...
   ```txt
   PASS components/UserCard.test.tsx
     ✓ renders user name and avatar (45 ms)
   ```

4. **コミットメッセージ生成依頼**

   ```bash
   claude "write a commit message for adding UserCard component and test"
   ```

   **出力例:**

   ```txt
   feat: add UserCard component and unit test
   ```

### 5. 参考リンク

- [Claude Code公式ドキュメント](https://docs.anthropic.com/en/docs/claude-code)
- [セットアップガイド](https://docs.anthropic.com/en/docs/claude-code/setup)
- [Claude Code - 47 PRO TIPS in 9 minutes](https://youtu.be/TiNpzxoBPz0)

---

## 2025-06-29: AWS Managed Services - RDS Deep Dive

### AWSにおけるマネージドサービスの基本

| 内容                     | オンプレ | MySQL on EC2 | Amazon RDS |
| ------------------------ | -------- | ------------ | ---------- |
| アプリからの利用         | ☑️       | ☑️           | ☑️         |
| バックアップ             | ☑️       | ☑️           |            |
| パッチ当て               | ☑️       | ☑️           |            |
...
- Undifferentiated heavy lifting
  - 事業に直接関係しない部分をAWSに任せることで、開発者は本来のビジネスロジックに集中できる

### AWSのマネージドサービスの例

- Amazon Relational Database Service (RDS)
  - MySQL、PostgreSQL、Oracle、SQL Serverなどのリレーショナルデータベースをマネージドで提供
  - 自動バックアップ、パッチ適用、スケーリングなどの機能を提供
- Elastic Load Balancing (ELB)
  - アプリケーションのトラフィックを複数のEC2インスタンスに分散
  - 高可用性とスケーラビリティを提供
...
- Amazon CloudFront
  ...
  - 静的および動的コンテンツの高速配信を実現
- Amazon Route 53
  - ドメインネームシステム (DNS) サービス
  - ドメイン名の登録、DNSルーティング、ヘルスチェックなどを提供
...
- **自動バックアップ**: データベースのバックアップを自動で取得し、指定した期間内で復元可能
- **スケーラビリティ**: データベースのサイズや性能を簡単にスケールアップ・ダウンできる
- **高可用性**: マルチAZ配置により、障害発生時の自動フェイルオーバーを実現
- **セキュリティ**: VPC内でのデータベース配置、IAMによるアクセス制御、暗号化などのセキュリティ機能を提供
- **パッチ管理**: データベースエンジンのパッチ適用を自動で行い、最新のセキュリティアップデートを適用
- **パラメータ設定**: RDSにはSSHアクセスがないため、パラメータグループを使用してデータベースの設定を管理

### Amazon RDSの利用シーン

- **Webアプリケーション**: ユーザーデータやセッション情報の保存
- **モバイルアプリケーション**: ユーザーアカウントやアプリ設定の管理
- **データ分析**: 大量のデータを効率的に保存・検索し、分析処理を行う
- **バックエンドシステム**: 業務アプリケーションのデータストレージとして利用

### Amazon RDSの利用方法

1. **RDSインスタンスの作成**
   - AWS Management ConsoleからRDSを選択し、インスタンスを作成
   - データベースエンジン（MySQL、PostgreSQLなど）、インスタンスタイプ、ストレージサイズを指定
2. **セキュリティグループの設定**
   - RDSインスタンスにアクセスするためのセキュリティグループを設定
   - 必要なポート（例: MySQLは3306）を開放し、アクセス元IPアドレスを指定
3. **データベースの接続**
   - 作成したRDSインスタンスのエンドポイントを取得し、アプリケーションから接続
   - データベースユーザー名とパスワードを使用して認証
4. **データベースの管理**
   - AWS Management ConsoleやCLIを使用して、データベースのバックアップ、スケーリング、パッチ適用などを管理
   - CloudWatchを利用して、パフォーマンスメトリクスの監視やアラート設定が可能

---

## 2025-06-28: Claude Code CLI クイックリファレンス

Claude Codeでの、生産性向上のための主要機能：
...
# 一回限りのコマンド実行
claude "Show me programming memes"
...
- **マルチファイル編集**: 一度のセッションで複数ファイルの読み取り、編集、作成が可能
- **コードベース理解**: プロジェクト全体の検索・分析
- **Git統合**: コミット、PR、ブランチ管理を自動化
- **テスト実行**: テストの実行と失敗の自動修正
- **ツールエコシステム**: Bash、ファイル操作、Web検索など

### 活用のコツ

- `/`で組み込みの操作コマンドを表示
- `--resume`で会話コンテキストを維持
- UI デバッグにはスクリーンショットを貼り付け
- 複雑な多段階リファクタリングタスクに対応
- IDE拡張機能との連携

### メモリ管理

- 会話履歴用に`.claude/`ディレクトリを作成
- プロジェクトの永続的なコンテキストには`CLAUDE.md`ファイルを使用
- 会話は自動保存され、再開可能

---

## 2025-06-27: Claude Code 始めてみた

巷で話題のClaude Codeを使い始めた。

### Claude Codeとは

[Claude Code](https://docs.anthropic.com/en/docs/claude-code)は、AnthropicのClaude AIモデルを活用したCLIツール。

ターミナルから直接Claudeとやり取りしながら、コーディング作業を効率化できる。
...
- ファイル構造を理解して的確な提案をしてくれる
- 複数のファイルにまたがる変更も一度に処理できる
- 既存コードの慣習を理解してそれに合わせたコードを書いてくれる
- Markdown形式で読みやすい出力

**注意点:**

- APIトークンの消費量は結構ある
- 大きなプロジェクトだとコンテキストが膨大になりがち
- セキュリティ上重要なファイルは自動で読まないよう注意が必要
...
# コードの修正依頼
Can you add error handling to the API endpoints?

# 新機能の実装
Help me implement user authentication using JWT
```

### まとめ

従来のGitHub CopilotやCursorとは異なり、プロジェクト全体を俯瞰しながらの作業が得意。

特に既存コードベースの理解と、それに適合した実装提案が優秀。

---

## 2025-06-26: AWS VPCネットワーク設計のベストプラクティス

AWS学習14日間コースの4日目（後半）で学んだVPCとサブネットの設計原則について。

特に将来的な拡張性とAWSサービスのIP使用量を考慮した設計手法を整理。

### VPC/サブネットのCIDR設計原則

**将来性を考慮した設計**：

- 将来必要になるプライベートIPアドレス数を事前に見積もる
- 基本的には余裕を持って大きめに設計（拡張は可能だが複雑）
- ただし、むやみに広いCIDRブロックを割り当てるのは非効率

**AWSサービスのIP消費量の把握**：

- 各AWSサービスが実際に使用するIPアドレス数を理解する
- 特にロードバランサーなどのスケーラブルなサービスの要件を確認

> ロードバランサーが正しくスケールできるように、ロードバランサーのアベイラビリティーゾーンサブネットごとに CIDR ブロックを最低でも /27 ビットマスク（例：10.0.0.0/27）にし、少なくとも各サブネットにつき8個の空き IP アドレスを用意してください。これらの 8 個の IP アドレスは、ロードバランサーが必要に応じてスケールアウトできるようにするために必要です。ロードバランサーはこれらの IP アドレスを使用して、ターゲットとの接続を確立します。それらがないと、Application
> Load Balancer ではノード交換の試行で問題が発生し、失敗状態になる可能性があります。

### 環境の境界を VPC で切るのはありか？

- なし。事故の温床になりうる。
  - 異なるシステム間はもってのほか
  - 同じシステムの本番環境とテスト環境を混ぜるのも避けたい

| 単位                   | メリット                                                                       | デメリット                                                                                        |
| ---------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| アカウント単位で分ける | - 他の環境のリソースが見えない - IAM の設定がシンプル                          | - アカウントごとに IAM 設定が必要                                                                 |
| VPC 単位で分ける       | - IAM の設定が 1 度でよい- 環境間でリソースを使い回せる？（むしろデメリット…） | - 各環境のリソースが見えるので作業しにくい、事故の元- それを避けるには複雑な IAM 設計・設定が必要 |

---

## 2025-06-25: Obsidian Web Clipperで情報収集ワークフローを再構築

Pocketの廃止に伴い、Obsidian Web
Clipperを導入して情報収集・管理ワークフローを見直した。単なる「後で読む」ツールから、「知識として蓄積・活用する」システムへの転換を図った。

### 導入背景と課題

**従来の問題点**：

- ブックマークやPocketに保存したページが「積読」になりがち
- 保存したページから得た知識が断片的で、他の情報と関連付けられない
- 検索しても「なぜ保存したか」の文脈が失われている

### Obsidian Web Clipperの実践的活用法

**1. ページクリップ時の工夫**：

- 関心を持った理由や期待する学びを簡潔にメモ
- 関連する既存ノートがあればその場でリンク作成
- プロジェクトや学習テーマに応じたタグ付け

**2. クリップ後の知識化プロセス**：

- 読了後に要点を自分の言葉で要約
- 他のノートとの関連性を意識したリンク構築
- 実践で使える形にアクションアイテム化

### 従来ツールとの比較優位性

| 機能           | Pocket/ブックマーク | Obsidian Web Clipper    |
| -------------- | ------------------- | ----------------------- |
| オフライン閲覧 | 限定的              | 完全対応                |
| ノート間リンク | なし                | 双方向リンク            |
| 検索の精度     | タイトル・タグのみ  | 全文検索 + コンテキスト |
| 知識の発展性   | 単発的              | 累積的・発展的          |
| カスタマイズ性 | 低い                | 高い（プラグイン拡張）  |

### 今後の展望

情報の「保存」から「活用」へのシフトにより、読んだ記事が単体で終わらず、既存の知識と有機的に結びつくワークフローを構築できた。

特に、プロジェクトベースの学習や技術調査において、過去の蓄積を効果的に活用できる基盤が整った。

---

## 2025-06-24: Zsh設定をGitで管理する方法

今日はzsh設定ファイル（`.zshrc`、`.zprofile`）をGitで管理するためのdotfilesリポジトリを構築した。
...
# Mise activation line
eval "$(mise activate zsh)"

# bun completions
[ -s "/Users/sungjoonpark/.bun/_bun" ] && source "/Users/sungjoonpark/.bun/_bun"
...
- **Zsh** - エイリアスとツール統合
- **Mise** - ランタイムバージョン管理（nvm、rbenv等の代替）
- **Bun** - JavaScriptランタイム・パッケージマネージャ
- **Homebrew** - macOSパッケージマネージャ
...
- **バックアップ** - 設定を失うリスクがない
- **同期** - 複数マシン間で同じ設定を使用可能
- **履歴管理** - 変更履歴を追跡、必要に応じてロールバック
- **共有** - チームや他の開発者と設定を共有
- **実験** - ブランチを使って新しい設定を安全にテスト
...
├── .git/
├── .gitignore
├── .zprofile       # Homebrew, OrbStack設定
├── .zshrc          # エイリアス, Mise, Bun設定
├── README.md       # ドキュメント
└── install.sh      # セットアップスクリプト
```

シェル設定がバージョン管理され、異なるマシン間で簡単に再現可能になった。
