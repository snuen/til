...
- Amazon EC2（Elastic Compute Cloud）は、AWSの"使いたいときに使いたいだけ使える"仮想サーバーサービス
  - オンデマンドでスケーラブルなコンピューティングリソースを提供
  - OSより上のレイヤーで、アプリケーションやサービスを自由に構築・運用できる
- Amazon EC2を使う上での4つのポイント
  1. OSの選択
     - Linux／Windowsなど、必要なOSを選べる
     - Amazon LinuxやUbuntuなど、AWSに最適化されたAMI（Amazon Machine Image）も利用可能
  2. インスタンスタイプの選択
     - CPU、メモリ、ストレージなどのスペックを選べる
     - 汎用／計算最適化／メモリ最適化など、用途に応じたインスタンスタイプが用意されている
  3. セキュリティグループ
     - インスタンスへのアクセス制御を行うファイアウォール
     - ポートやIPアドレスでアクセスを制限できる
  4. 秒単位の従量課金
     - 利用した時間だけ課金される
     - スポットインスタンスを使えば、さらにコストを抑えられる

### EC2インスタンスの起動

1. EC2ダッシュボードから「インスタンスの起動」を選択して起動する
2. AMIを選択
   - Amazon Linux 2やUbuntuなど、必要なOSを選ぶ
3. インスタンスタイプを選択
   - 汎用（t3.microなど）や計算最適化（c5.largeなど）から選ぶ
   - t3.microは無料利用枠対象
4. インスタンスの詳細設定
   - VPCやサブネット、IAMロールなどを設定
   - デフォルトで問題なければそのまま進む
5. ストレージの追加
   - デフォルトのEBSボリュームサイズを確認・変更
   - 必要に応じて追加のEBSボリュームを設定
6. タグの追加
   - インスタンスに名前を付けるなど、管理用のタグを追加
7. セキュリティグループの設定
   - 新規作成または既存のセキュリティグループを選択
   - SSH（22番ポート）やHTTP（80番ポート）など、必要なポートを開放
8. キーペアの選択
   - 新規作成または既存のキーペアを選択
   - キーペアはSSH接続に必要なので、必ずダウンロードしておく
9. インスタンスの起動
   - 設定内容を確認し、インスタンスを起動
...
```sh
# キーペアを指定してEC2インスタンスにSSH接続
ssh -i /path/to/your-key.pem ec2-user@your-instance-public-ip

# WARNING: UNPROTECTED PRIVATE KEY FILE! と出たら

# キーペアのパーミッションを変更する必要がある
chmod 400 /path/to/your-key.pem

# うまくSSH接続できたら、下記のようなログが出てくる
# Amazon Linux 2023
```

ミドルウェアとして、Apacheのインストール例：

```sh
sudo dnf update -y # パッケージの更新
sudo dnf install httpd # Apacheのインストール
dnf list --installed | grep httpd # インストール確認
```

HTTP接続も許可している場合、Public IPv4アドレスにブラウザからアクセスすると、Apacheのデフォルトページ「It
works!」が表示される。

### AMIを作成し、AMIからEC2インスタンスを起動する

- AMI (Amazon Machine Image)
  - EC2インスタンスのイメージファイル（作る元で、断面とも表現できる）
    - OSやアプリケーション、設定を含む。
- Amazon EBS snapshot
  - ボリューム（EBS）の断面
  - EBSボリュームのバックアップ（AMIの一部）
  - EBSを作る元となる

### Elastic IPアドレスを使って、EC2インスタンスに固定IPを割り当てる

- Elastic IPアドレス
  - AWSが提供する静的なIPv4アドレス
  - EC2インスタンスに割り当てることで、インスタンスの再起動や停止・起動後も同じIPアドレスを維持できる
