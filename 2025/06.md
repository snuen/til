...
- プロダクトごと、環境ごとに分けるのが一般的
- プロダクト単位、環境単位での利用料の請求がくる
...
- AWSの利用料金利用料金について
  - 各サービスの利用料金は、公式Webサイトで確認できる
  - 無料利用枠について
  - 見積もり方法
- 料金アラートを設定しておく
  - 予算を設定しておくと、予算を超えた場合に通知が来る
- どのアカウントでどんな支払いを行うかは抑えておくとよい
  - 会社の法人カードを登録するのか
  - 請求書払いにするのか
...
- ルートユーザー
  - すべてのAWSサービス・すべての操作が可能な特権ユーザー
  - アカウント全体設定、サポートプラン変更などのときのみ使用
- IAMユーザー
  - AWSサービスの利用者
  - IAMユーザーごとにアクセス権限を設定できる
  - アクセスキー・シークレットキーを発行してAPIアクセスも可能
  - 利用者ごとごとに払い出し、通常の作業はこのIAMユーザーで行う
...
- AWS CloudTrail
  - AWS上での操作ログを記録するサービス
  - デフォルトで有効
  - Amazon S3に証跡を残す設定が可能
- Amazon GuardDuty
  - AWS上の脅威を検出するサービス
  - 悪意のあるアクティビティがないかを確認できる
  - 検出した脅威をAmazon SNSで通知する設定が可能

AWSにおけるセキュリティをもっと詳しく知るには、AWS Well-Architected Frameworkのセキュリティの柱を読むとよい。
